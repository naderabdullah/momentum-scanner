// components/scanner/StatusBar.tsx
...
const StatusBar: React.FC<StatusBarProps> = ({ marketStatus, stockCount, apiCalls, catalystCount, lastUpdate }) => {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 p-4 bg-slate-900/50 border-b border-slate-800">
      ...
      <div className="panel text-center p-3 rounded-lg"><strong className="block text-slate-400">Last Update</strong><span id="lastUpdate" className="text-lg font-bold">{lastUpdate}</span></div>
    </div>
  );
};
